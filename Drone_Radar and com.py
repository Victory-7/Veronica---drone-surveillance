...
COMMUNICATION_RANGE = 200  # Master drone communication range
...
while running:
    ...
    master_drone_x, master_drone_y = drones[MASTER_DRONE_INDEX]
    ...
    # Master Drone Communication
    for i in range(NUM_DRONES):
        if i != MASTER_DRONE_INDEX:
            slave_x, slave_y = drones[i]
            comm_distance = math.sqrt((master_drone_x - slave_x) ** 2 + (master_drone_y - slave_y) ** 2)
            if comm_distance <= COMMUNICATION_RANGE:
                pygame.draw.line(screen, PURPLE, (master_drone_x, master_drone_y), (slave_x, slave_y), 1)
