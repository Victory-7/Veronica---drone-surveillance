...
pygame.display.set_caption("Drone Swarm Simulation with Radar and Communication")
...
PACKET_LOSS_PROBABILITY = 0.1  # 10% chance of message loss
LATENCY_RANGE = (10, 100)  # Latency in milliseconds

# ACK system for message confirmation
...
while running:
    ...
    master_x, master_y = drones[MASTER_DRONE_INDEX]
    for i in range(NUM_DRONES):
        ...
        # Master drone communicates with nearby drones
        if i != MASTER_DRONE_INDEX:
            comm_distance = math.sqrt((drone_x - master_x) ** 2 + (drone_y - master_y) ** 2)
            if comm_distance <= RADAR_RANGE / 2:
                # Simulate packet loss
                if random.random() > PACKET_LOSS_PROBABILITY:
                    latency = random.randint(*LATENCY_RANGE)
                    pygame.time.delay(latency)
                    pygame.draw.line(screen, YELLOW, (int(master_x), int(master_y)), (int(drone_x), int(drone_y)), 1)
                    acks[i] = True  # Message received successfully
                else:
                    acks[i] = False  # Message lost

    # Display communication status
    for i in range(1, NUM_DRONES):
        status = "Received" if acks[i] else "Lost"
        pygame.draw.circle(screen, GREEN if acks[i] else RED, (10 + i * 20, 10), 5)
        pygame.font.Font(None, 20).render(status, True, BLACK)
