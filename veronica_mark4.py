...
while running:
    ...
    # Draw obstacles
    for ox, oy, radius in OBSTACLES:
        pygame.draw.circle(screen, NEON_RED, (ox, oy), radius, 2)

    # Draw radar circle
    pygame.draw.circle(screen, NEON_BLUE, (radar_x, radar_y), RADAR_RANGE, 1)
    ...
    pygame.draw.line(screen, NEON_GREEN, (radar_x, radar_y), (radar_x_end, radar_y_end), 2)
    ...
    for i, drone in enumerate(drones):
        ...
        if not drone.has_stealth:
            ...
            if distance <= RADAR_RANGE:
                ...
                pygame.draw.circle(screen, NEON_GREEN, (int(drone.x), int(drone.y)), 5, 1)
        
        # EMP effect
        if drone.has_emp and drone.emp_cooldown > 0:
            pygame.draw.circle(screen, NEON_RED, (int(drone.x), int(drone.y)), 8, 2)

    # Display fading blips
    for blip in blips:
        pygame.draw.circle(screen, NEON_GREEN, blip, 3)
